...
```{r import data}
data_2020 <- read_excel(here("data/xlsx", "2020-21 Professional Personnel Individual Staff Report.xlsx"))
...
```{r apply select func}
data_list <- list(data_2014, data_2015, data_2016, data_2017, data_2018, data_2019)

temp <- lapply(data_list, function(x){x <- x %>% dplyr::select(SY, PublicID); return (x)})

names(temp) <- c("data_2014_short", "data_2015_short", "data_2016_short", "data_2017_short", "data_2018_short","data_2019_short")
...
```{r join datasets}
data_14_19_short <- data_2014_short %>%
...
data_14_19_short %>% head()

data_14_19_short <- data_14_19_short %>%
  mutate(Y14 = SY.x, Y15 = SY.y, Y16 = SY.x.x,
         Y17 = SY.y.y, Y18 = SY.x.x.x, Y19 = SY.y.y.y) %>%
  select(PublicID, Y14:Y19)
```
...
```{r}
sum(data_14_19_short$Y14)
sum(data_14_19_short$Y15)
sum(data_14_19_short$Y16)
sum(data_14_19_short$Y17)
sum(data_14_19_short$Y18)
sum(data_14_19_short$Y19)
```
